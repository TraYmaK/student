...
BUILD_OUTPUT=$(cmake --build . --target tests lab1m 2>&1)
BUILD_RESULT=$?
...
./tests --gtest_color=no 2>&1 | \
    ...
    { 
        if (/^Restoration|^Using|^=== Organizing|^Preparing|^Mounting|^Setting|^Exhibition|^Climate|^Visitor|^==== GALLERY|^Select|^Running main|^$/) {
            next
        }
    }
    ' || TEST_RESULT=$?
